...
namespace StarPie.Kernel.ShellIntegration
{
    ...
    /// </summary>
    /// <remarks>
    /// 只有一个对象：首实例发布并持有到进程结束的**标记事件**。它的名字编码发布者的形态与权限态，
    /// 因而一个对象同时担三件事——"同形态首实例在此"（存在即真）、"它是否提权"（<c>_High</c> /
    /// <c>_Normal</c>）、"请让位"（提权新实例对它置位，首实例等到即走既有退出编排）。
    /// 一个对象够用，是因为接管的方向只有一个：只有提权新实例会置位，也只有非提权首实例会等它。
    /// dev 实例的标记名带独立后缀（与计划任务名、配置目录同口径）：不同形态共用单实例互斥体，
    /// 却不共用握手对象，故接管对它们不可能成立——读不到同形态标记即按置前退出走。
    /// **就绪判据只有"单实例互斥体已可取得"这一个来源**（见 <see cref="WaitForSingleInstanceRelease"/>），
    /// 不取自任何触发命令的退出码——实测该退出码只表示"任务被受理"，动作为空时同样返回成功。
    /// 标记事件的句柄归首实例（互斥体与它同生共死），第二个实例只按名字探测与置位。
    /// </remarks>
