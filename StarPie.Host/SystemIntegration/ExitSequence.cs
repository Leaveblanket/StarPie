...
namespace StarPie.Host.SystemIntegration
{
    ...
    /// <summary>
    /// 托盘退出的固定顺序（纯决策，无输入参数）：落盘 → 释放托盘 → 应用关闭。
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public static class ExitSequence
    {
        /// <summary>托盘退出的有序步骤（调用方持有全部动作）。</summary>
        public static IReadOnlyList<ExitStep> Resolve() => new[]
        {
            ExitStep.FlushPendingSave,
            ExitStep.ReleaseTray,
            ExitStep.ShutdownApplication,
        };
    }
}
