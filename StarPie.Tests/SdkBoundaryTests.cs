...
/// 全仓程序集导出类型唯一（不出现同类型双份定义），被掏空的六个契约工程暂留为空壳
/// （P1.3/#112 纯托管三件 + P1.4/#113 WPF 三件），源码树保持「镜像旧相对路径」的过渡形态。
/// 与 <see cref="SdkWpfBoundaryTests"/>（WPF 面）、<see cref="FourSetBoundaryTests"/>（工程面）、
...
public sealed class SdkBoundaryTests
{
    ...
    [Fact]
    public void 被收口契约工程_暂留为空壳()
    {
        string[] shells =
        {
            // P1.3/#112：内容迁 StarPie.Sdk
            "StarPie.Dialogs.Contracts", "StarPie.Gestures.Contracts", "StarPie.Wheel.Contracts",
            // P1.4/#113：内容迁 StarPie.Sdk.Wpf
