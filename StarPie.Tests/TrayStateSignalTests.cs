...
public sealed class TrayStateSignalTests
{
    ...
    public void 关闭设置台_固定顺序_落盘先于任何释放动作且出账先于GC()
    {
        IReadOnlyList<TraySignalStep> steps = TrayStateSignal.Resolve(
            TrayStateChange.ConsoleClosed, isExiting: false);
    ...
    {
        IReadOnlyList<TraySignalStep> steps = TrayStateSignal.Resolve(
            TrayStateChange.ConsoleOpened, isExiting: false);
    ...
    {
        Assert.Empty(TrayStateSignal.Resolve(TrayStateChange.ConsoleClosed, isExiting: true));
        Assert.Empty(TrayStateSignal.Resolve(TrayStateChange.ConsoleOpened, isExiting: true));
    }
}
