...
/// <para>
/// <b>已知边界</b>：本类不判定"关闭后窗口与视图树对象可回收"。已显示窗口经 <c>Close()</c> 后
/// HWND 已销毁、已退出全局窗口集合、主窗口属性已改写，但该 <see cref="Window"/> 托管对象会留在堆上
/// 直到下一次 gen2 回收——无强根，属回收延迟；<c>x:Name</c> 生成字段随窗口对象存活，故窗口的视图树
/// 与 DataContext 在此期间仍可达。回收时机不确定，加上本类夹具自身持有窗口强引用，使即时弱引用断言
/// 不可判定，故不在本层断言，改由结构不变量（INV1/INV3）与常驻内存测量承担。
/// </para>
/// </remarks>
