...
public sealed class IconPickerViewModelTests
{
    ...
    private static CustomIconItem Custom(string key, string displayName, bool isSvg = false)
    ...
    {
        var vm = Create(
            customs: new List<CustomIconItem> { Custom("custom:star", "star") },
    ...
    {
        var vm = Create(
            customs: new List<CustomIconItem> { Custom("custom:star", "MyStar"), Custom("custom:moon", "月亮") },
            vectors: new List<VectorIconItem> { Vector("Copy", "复制 (Copy)") });
    ...
    {
        var customs = new List<CustomIconItem> { Custom("custom:old", "old") };
        var dialogs = new TestDialogService { OpenFileToPick = new FilePickResult(@"C:\icons\new.svg") };
        var vm = Create(
            customs: customs,
            dialogs: dialogs,
            // 注入的导入委托成功后，测试同样把新图标并入来源列表。
    ...
    {
        var customs = new List<CustomIconItem> { Custom("custom:star", "star") };
