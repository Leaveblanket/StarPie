...
using StarPie.Host.WheelGesture;
using StarPie.Sdk.Models;

namespace StarPie.Ui.Services.Input
{
    /// <summary>
    /// 输入栈的捕获侧：全局鼠标钩子把触发键的按下 / 移动 / 抬起喂入 <see cref="WheelGestureEngine"/>，
    /// 按引擎决策同步抑制事件，并把引擎交付的副作用（补发点击 / 执行动作）卸载到 UI 线程。
    /// 轮盘手势决策全部在引擎内，本类只做事件捕获、抑制与副作用投放。
    /// </summary>
    /// <remarks>
    ...
    /// 外部注入（e2e 的 pywin32 注入）必须仍能触发轮盘手势。
