...
namespace StarPie.Modules
{
    ...
    /// <c>StarPie.Icons.Contracts</c>；.lnk 解析契约 <see cref="IShortcutTargetResolver"/> 在 #95
    /// 中间态暂留 Core（#96 随 Programs.Contracts 迁出后本 runtime 改经契约边，Core 引用随之删除）。
    /// 本模块无导航页，不提供 RegisterNavigation。
    /// </remarks>
    public static class IconsModuleRegistrar
    {
        /// <summary>注册 S1 图标资产实例服务（容器单例）：工厂经 ServiceProvider 惰性解析 Core 的
        /// <see cref="IShortcutTargetResolver"/>（由 ProgramsModuleRegistrar 注册的 M3 实现提供）。</summary>
