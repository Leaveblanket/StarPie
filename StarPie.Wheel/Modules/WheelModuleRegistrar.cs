...
namespace StarPie.Modules
{
    ...
    /// 与接口 <see cref="IWheelFactory"/> 同驻本模块，手势侧只经该接口消费，不反向组装
    /// 瞬态轮盘；预览 Profile 只读契约 <see cref="IProfilePreviewSource"/> 位于共享内核，
    /// 实现方与消费方经其解耦。工厂/外观子 VM 只依赖共享内核契约与
    /// <see cref="IThemeService"/>（允许边），不反向引用宿主。
    /// </remarks>
    public static class WheelModuleRegistrar
    {
        ...
        public static void RegisterServices(IServiceCollection services)
        {
            ...
            // 轮盘外观设置子 VM：单例注入外观聚合 VM；预览 Profile 上下文经共享内核只读契约
            // IProfilePreviewSource 转发（实现方别名由 GesturesModuleRegistrar 注册），
            // 本注册器只解析共享内核契约，不引用具体方案列表 VM。
