...
namespace StarPie.Modules
{
    ...
    /// 与接口 <see cref="IWheelFactory"/> 契约（ADR-0023/#97 起驻 StarPie.Wheel.Contracts）
    /// 本模块实现之，手势侧（Gestures runtime）只经该契约消费，不反向组装瞬态轮盘；
    /// 预览 Profile 只读契约 <see cref="IProfilePreviewSource"/> 随实现方 M1 下沉
    /// StarPie.Gestures.Contracts，实现方与消费方经其解耦。工厂/外观子 VM 只依赖共享内核、
    /// 各自契约程序集与 Theme.Contracts 的 <see cref="IThemeService"/>（M2→M4 runtime
    /// 允许边清零），不反向引用宿主。
    /// </remarks>
    public static class WheelModuleRegistrar
    {
        ...
        public static void RegisterServices(IServiceCollection services)
        {
            ...
            // 轮盘外观设置子 VM：单例注入外观聚合 VM；预览 Profile 上下文经 Gestures.Contracts
            // 只读契约 IProfilePreviewSource 转发（实现方别名由 GesturesModuleRegistrar 注册），
            // 本注册器只解析契约程序集，不引用具体方案列表 VM。
