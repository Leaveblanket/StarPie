...
def test_launch_action_run_as_admin_option_roundtrip(app):
    """
    启动动作的显式提权选项落盘往返（ADR-0040 决策 6）：类型选 Launch 后勾选框出现，
    勾选并 Save 后 Global 首个动作 RunAsAdmin=true；取消勾选再 Save 回落 false。
    默认未勾选即"提权态经 Explorer 中介降权"，故该键必须能显式往返。
    """
    win, local_app_data = app

    # 1. Switch to Page 2 (Gestures & Actions)
    goto(win, 2)

    # 2. 类型切到 Launch：勾选框所在行随 IsLaunchType 显示（非 Launch 时该行不在 UIA 树内）
    type_combo = win.child_window(auto_id="Slot0ActionTypeComboBox", control_type="ComboBox")
    assert type_combo.exists(timeout=3), "Slot0ActionTypeComboBox 必须存在（首个槽位动作类型）"
    select_option(type_combo, SLOT_ACTION_TYPE_CATALOG, "Launch", verify_selection=False)

    # 3. 勾选"以管理员身份启动"（Toggle 失败必须红，不吞异常）
    run_as_admin_chk = win.child_window(auto_id="Slot0RunAsAdminCheckBox", control_type="CheckBox")
    assert run_as_admin_chk.exists(timeout=3), "Slot0RunAsAdminCheckBox 必须存在（启动动作的提权选项）"
    pre_state = run_as_admin_chk.get_toggle_state()
    run_as_admin_chk.toggle()
    toggled_state = not bool(pre_state)

    # 4. Save 后按落盘值断言（UIA 选中态回读不作门，config.json 才是权威）
    win.child_window(auto_id="SaveButton", control_type="Button").invoke()

    def _first_global_action(c):
        profiles = c.get("Profiles", [])
        glob = next((p for p in profiles if p.get("ProcessName") == "Global"), None)
        actions = (glob or {}).get("Actions") or []
        return bool(actions) and actions[0].get("Type") == "Launch" \
            and bool(actions[0].get("RunAsAdmin")) == toggled_state

    config = read_config(local_app_data, predicate=_first_global_action)
    glob = next(p for p in config["Profiles"] if p.get("ProcessName") == "Global")
    first = glob["Actions"][0]
    assert first["Type"] == "Launch", f"动作类型应为 Launch，got {first.get('Type')}"
    assert bool(first.get("RunAsAdmin")) == toggled_state, \
        f"RunAsAdmin 应为 toggle 后的状态（toggle 前 state={pre_state}），got {first.get('RunAsAdmin')}"
