...
>
> 目标态程序集与文件架构见 [plugins.md](../architecture/plugins.md)。本 ADR 部分推翻 [ADR-0023](0023-module-contracts-hard-boundary-and-core-narrowing.md) 的**对外**契约判据（内部模块边界判据保留）。

## 动机

1. **现状是编译期模块，不是插件**：Host 在 `Composition.cs` 静态调用全部模块注册器、在 `App.xaml` 静态合并模块页面模板与主题字典、`NavigationCatalog` 是封闭槽位且启动时 `Validate()`、组合根在启动期 eager 解析全部页面 VM。模块可以拆集，但没有任何运行期装卸入口。
2. **第三方生态需要单一 SDK 面**：多程序集形态下，外部作者要消费宿主能力需引用共享内核加多个薄 `*.Contracts` 并理解内部模块编号；模块 runtime 以契约互引、宿主引用全部 runtime。这不是可发布、可版本化、可承诺兼容的 SDK。
...
- **维持模块集现状、增量加 SDK**：假边界与 Host 静态引用不消，插件仍能绕过 SDK 引内部实现，ABI 不可控 → 否。
- **沿用"契约随实现方下沉"作为插件契约判据**：契约按模块分散，对外引用面随内部改集漂移 → 否（该判据作为 Host 内部目录规则保留）。
- **进程外插件**：真隔离、可强杀卸载，但 UI/性能/调试/生态成本高，与"进程内真卸载"的决定冲突 → 否，保留为未来承载不可信插件的后端。
- **允许插件提供 WPF 视图/XAML**：需要把 Application 资源树、DataTemplate、导航目录全部动态化，且卸载可靠性无法在进程内形态下证明 → 否。
- **三集 + headless 能力插件** → 采纳。

## Decision

1. **物理形态三集**：`StarPie.Sdk`（唯一第三方引用面：零 WPF、零第三方包依赖）、`StarPie.Host`（宿主内核：全部非 XAML 运行时、服务与插件宿主）、`StarPie.Ui`（WinExe：App/ShellHost/Composition + 全部 ViewModel/View/Window/XAML/主题字典）；发布产物仍名为 `StarPie.exe`。
...
6. **依赖自治**：插件携带私有依赖经 `AssemblyDependencyResolver` 解析；不允许插件间互相依赖，避免级联卸载与版本地狱。
7. **程序集收敛**：原各模块与契约程序集的职责并入三集 + 插件；`StarPie.Shell`/`StarPie.Theme`/`StarPie.Dialogs`/`StarPie.Icons`/`StarPie.Programs` 与各 `*.Contracts` 不作为独立程序集存在（逐集去向见 [assemblies.md](../architecture/assemblies.md)）。
...
- 程序集边界断言随之改写为「三集 + 插件 ABI/依赖」口径，`modules.md`/`assemblies.md` 按三集形态重述。
