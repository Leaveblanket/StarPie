...
> [ADR-0013](./0013-localization-theme-overhaul.md)（#41 起）取代决策 1（`I18n` 静态唯一源 + 不引入 `ILocalizationService`）：本地化数据与取词改 `resx` + `ILocalizationService` 实例服务，静态 `I18n` 于 S4（#45）删除；其余条款（文案四分类、VM 生命周期契约、e2e AutomationId）维持不变。

5 个设置页的 code-behind 回填已改为“应用级语言字典 + `{DynamicResource}`”（#29）。剩余对话框/壳层/托盘/VM 的 `I18n.T()` 消费需要统一的**目标态原则**，否则每张迁移票都会自造一套机制。本 ADR 按“文案本质”而非“代码层”立法，承接 ADR-0002/0005/0008/0009。

## 术语

术语定义已并入 CONTEXT.md 与 [ADR-0013](./0013-localization-theme-overhaul.md)（决策 2 维持四类文案分类）。

## 决策

1. **语言变更源与介质不变量**
   - `I18n` 静态 + `LanguageChanged` 是唯一语言变更源（ADR-0002 判据不动）；
   - 应用级语言字典只是“声明式文案”的介质，不是给 VM/托盘用的 API；
   - 不引入每语言 XAML 资产文件，不引入 `ILocalizationService`。

2. **文案分类与各自机制**
   - **声明式文案**：页面（已迁）+ 壳层（`MainView`/`SidebarView`）+ 对话框（Color/Icon/Program/Input）的静态文本一律 `{DynamicResource}`；例外只保留 XAML 表达不了的位置——`Window.Title` 拼接、托盘 tooltip、View 动态生成项（注释指向本 ADR / ADR-0009）。`Window.Title` 收进壳层 VM 属性（`MainViewModel.WindowTitle`）后不再是例外。
...
4. **内容与机制分离**：未本地化硬编码文案（页面副标题、About 里程碑、屏上取色提示等）另立“文案缺口盘点”backlog，机制迁移票不新增翻译键（`I18n.Translations` 数据维持不动）。
