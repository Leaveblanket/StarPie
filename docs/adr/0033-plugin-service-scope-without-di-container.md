...
> Status: Active
>
> 契约正典：[plugins.md §6](../architecture/plugins.md)、[plugin-contracts.md §4](../architecture/plugin-contracts.md)（§4 第 4 条按本 ADR 修订）。

## 动机

1. 目标态写法（ADR-0027 决策 3、plugin-contracts.md §4 约束 4）是"每插件子 `ServiceProvider`"，其要义是**作用域隔离**：宿主根容器不含插件类型，插件实例只活在按插件划分的边界内。
...
3. 撤销"子 `ServiceProvider`"中的容器承诺，保留其隔离承诺：宿主根容器不含任何插件类型这一条不变（Ui 组合根也不注册插件类型）。plugin-contracts.md §4 第 4 条按此修订。
