...
- **SharpHook `EventLoopGlobalHook` / `TaskPoolGlobalHook`**:否决。抑制必须与钩子同线程同步设置,这两者会忽略抑制——而抑制是手势的地基。
