...
- **SharpHook `EventLoopGlobalHook` / `TaskPoolGlobalHook`**:否决。抑制必须与钩子同线程同步设置,这两者会忽略抑制——而抑制是轮盘手势的地基。
...
- **raw input**(微软对该场景的替代建议):否决。raw input 只能异步监视、不能抑制,与「吞掉触发键」的轮盘手势语义冲突。
