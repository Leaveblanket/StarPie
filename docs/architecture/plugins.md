...
- as-built：**提权态**页首另有一行警示（`PluginManagerElevatedNotice`，ADR-0040 决策 6）——插件为进程内
  加载、宿主不承诺沙箱与配额（ADR-0029），宿主提权即把第三方插件的爆炸半径从用户级抬到机器级；
  非提权态整行不出现。提权态不拒绝加载插件：准入决策已交给用户（ADR-0029），不因权限级别被二次否决。
