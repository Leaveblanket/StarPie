...
1. **主题**：`ThemeService` 单例；`ApplyTheme` 为窗口/元素应用画刷与 DWM 深色模式；`ResolveEffectiveTheme` 解析 `System`/空值（经注入探测委托读注册表实时判定系统深浅色）；页面不持 `IThemeService`，仅 `MainView`/对话框窗口构造注入做白名单应用（[ADR-0009](../adr/0009-view-code-behind-whitelist.md)）。
