...
└── StarPie.Tests/          # xUnit 单测（显式引用四集、Core（设计期投影壳）与 Dialogs、Shell、Gestures 模块程序集）
```
...
| `Modules/` | exe 内 Host 外观聚合页：`HostModuleRegistrar`（RegisterNavigation）+ `HostPageTemplates.xaml` 与 M4 `ThemeModuleRegistrar`、M2 `WheelModuleRegistrar`（RegisterServices，均无导航页）；`StarPie.Shell/Modules/`：`ShellModuleRegistrar` + `ShellPageTemplates.xaml`；`StarPie.Gestures/Modules/`：`GesturesModuleRegistrar` + `GesturesPageTemplates.xaml`；其余模块注册器在各自工程 `Modules/` | 不承载业务；注册器只注册不解析 |
...
- `StarPie.Host.csproj` / `GlobalUsings.cs`：宿主内核工程入口（net10.0 零 WPF；ProjectReference
  ...
  解析）与工程级
  `GlobalUsings.cs`——目标树其余目录（`Actions/`、`Gestures/`、
...
- 各工程源码根目录**只允许**上表与本小节列出的项；原型、HTML、临时脚本不得留在
  ...
  `StarPie.Dialogs/`、`StarPie.Shell/` 或 `StarPie.Gestures/` 下。
