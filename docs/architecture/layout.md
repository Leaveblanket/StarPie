...
├── StarPie.Gestures/              # M1 手势与动作模块程序集（WPF 类库，程序集 StarPie.Gestures；命名空间 WinPieGestures.*，B10 收口；B9/#82 起；单向 Core + M2 允许边）
│   ├── StarPie.Gestures.csproj    # SDK 工程文件（RootNamespace=WinPieGestures；引用 Core + Wheel）
...
│   └── Views/Pages/               # M1：TriggerSettingsPage、GesturesSettingsPage（根基类 SettingsPageBase 在 Core）
└── WinPieGestures.Tests/          # xUnit 单测（显式引用 Host、Core、Programs、Shell、Theme、Wheel 与 Gestures）
```
...
> 其余业务目录（Host 外观聚合页/对话框实现等）留 `WinPieGestures/`。
...
### 模块程序集目录（B4/#77 起；B6/#79 起含首个带 DI 的模块程序集；B7/#80 起含 M4；B8/#81 起含 M2；B9/#82 起含 M1）
...
| `StarPie.Gestures/Views/Pages/` | M1 页面 View：`TriggerSettingsPage`、`GesturesSettingsPage`（根基类 `SettingsPageBase` 在 Core `Views/Pages/`，XAML xmlns 经 `assembly=StarPie.Core` 引用） | 不注册容器；不编排业务/写配置/调服务；页面无参构造 |
...
- `StarPie.Gestures.csproj` / `GlobalUsings.cs`：M1 手势与动作模块程序集工程入口（B9/#82 起，
  单向引用 Core + 允许引用 Wheel（M1→M2 允许边））；`StarPie.Gestures/` 源码根目录**只允许**
  ...
  `ViewModels/Pages/`（BehaviorSettingsViewModel/ProfileListViewModel）与 `Views/Pages/`
  （TriggerSettingsPage/GesturesSettingsPage）。
- 各工程源码根目录**只允许**上表与本小节列出的项；原型、HTML、临时脚本不得留在
  `WinPieGestures/`、`StarPie.Core/`、`StarPie.Programs/`、`StarPie.Shell/`、
  `StarPie.Theme/`、`StarPie.Wheel/` 或 `StarPie.Gestures/` 下。
...
- **B9/#82（2026-09-06）**：M1 手势与动作迁入独立模块程序集 `StarPie.Gestures/`——手势管线
  ...
  GesturesSettingsPage、SlotViewModel）随迁（命名空间不变，B10 统一收尾）；exe 内 M1 临时注册器/
