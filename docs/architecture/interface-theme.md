...
- `StarPie.Theme/AppThemePaletteManager.cs`（模块根，主题调色板整项替换；裁决 public——Host
  `AppHost` 装配面，同 `TrayIconManager` 先例，见 [assemblies.md](assemblies.md) §7）。
...
3. **宿主编排（H1 放行面）**：`AppHost` 只编排（宿主流程见 [host.md](host.md)）：构造时
   `new AppThemePaletteManager()`（StarPie.Theme public 装配面）并
...
4. **界面主题设置面（ADR-0014 决策 6/7）**：`InterfaceThemeSettingsViewModel`
   （`StarPie.Theme/ViewModels/Pages`，DI 单例，由 `ThemeModuleRegistrar.RegisterServices`
