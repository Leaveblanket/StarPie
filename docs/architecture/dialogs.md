...
- **S6 契约（`StarPie.Dialogs.Contracts/`，ADR-0023/#96；纯 C#）**：
...
- **实现与界面（独立模块程序集 `StarPie.Dialogs/`，ADR-0020/#88）**：
...
1. `DialogService` 构造注入 `IThemeService`、`ILocalizationService`、共享图标资产实例服务
   ...
   下放——ADR-0020/#88/ADR-0023/#96：契约随实现方驻契约程序集，组合根不再直调 M3 静态扫描，
