...
ResidentShell 回填）属宿主侧装配职责；本文件以下分层规则适用于各程序集内部。
...
1. **Views → Services 白名单**：View 构造可注入 `IThemeService` 仅用于窗口界面主题应用（View code-behind 白名单第 5 条）；不得注入业务服务、配置服务或在 View 中调用服务方法。深浅色读数走宿主注入的无状态 `Func<bool>` 探针，不是服务依赖（轮盘与托盘消费该探针）。
...
- **命名空间 = 工程名 + 工程内相对目录**：命名空间根是**工程名**（即 csproj 文件名），
  ...
  根级类型（`App`、`ResidentShell`、`SettingsConsole`、`Composition`、`DevInstance`）在 `StarPie.Ui`。
...
- **可见性**：
  ...
  - 需要被宿主装配的模块公开件显式 `public`；与装配方同集、只作容器解析或被测类型的件维持 `public`，装配方与实现同集且无跨集消费的回落 `internal`。
...
- 页面 View 无参构造、不注册容器，因此不需要 public 构造注入（`SettingsConsoleWindow`、对话框 Window、轮盘 `RadialWindow`（由 `WheelFactory` 组装）是仅有的、经组合根/服务显式 `new` 的窗口）。
...
- **只由组合根注册（经内置贡献者 RegisterServices 登记）**；View/ViewModel 不自行 `new` 服务、
  不使用服务定位器（导航执行入口 `NavigationExecutor` 例外见上——宿主内部解析缝）。
...
- **图标资产双形先例**：有状态/IO/Win32 面（自定义图标存储缓存、文件/程序
...
- 主框架 VM 拆分：`NavigationViewModel`（导航状态；目录驱动；运行时主体在
  ...
  `WindowChromeViewModel`（窗口标题/退出态/保存，宿主窗口外框）分别供 `SettingsConsoleWindow` 分区 DataContext 的
  导航区与窗口外框。
