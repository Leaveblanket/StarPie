...
3. **契约归属（ADR-0023 内部判据 + ADR-0027 决策 7）**：模块出口契约（接口 + 跨模块 DTO/纯数据）入跨集
   共享的契约面 `StarPie.Sdk`/`StarPie.Sdk.Wpf`（SDK 单一引用面）；内部边界判据（模块 runtime
   零互引、契约不反向依赖）继续有效。共享件出现第二个消费方族时——若属全局
   机制/数据入 `StarPie.Sdk` 契约与模型面（跨集共享）或宿主内核（运行时设施），若属某模块出口契约则同样入契约面（单一消费方的能力留在消费
...
- 共享视图基础设施（放行面不持有 UI 实现件）：通用共享转换器与
  全局控件样式字典 `ModernControls.xaml` 落 `StarPie.Ui/Views/Converters/` 与 `StarPie.Ui/Views/Styles/`——App.xaml 仍为单点
...
- **关键内部**：`WheelViewModel`（实现 `IWheelViewModel`）、`WheelAppearanceSettingsViewModel`
  ...
  `StarPie.Sdk`，ADR-0023）；物理路径见 [layout.md](layout.md) 与
  [wheel.md](wheel.md)。
...
- **关键内部**：配置服务与保存编排（`IConfigService`/`JsonConfigService`/`ISaveDebouncer`/
  `SettingsSaveOrchestrator`/`AppDataPaths`，物理居宿主内核 `StarPie.Host/Configuration/`；
...
- **关键内部**：宿主内核 `StarPie.Host/Localization/`（`ILocalizationService`/`LocalizationService` + `Strings*.resx`）；设计期投影字典 `DesignTimeStrings.xaml` 与生成脚本在 `StarPie.Ui/Services/Localization/`；`ShellHost` 的语言字典投影是 H1 对本模块的消费（§5 D4）。
...
- **关键内部**：契约 `IDialogService` + 结果 record 驻 `StarPie.Sdk/Services/Dialogs/`
  ...
  （`DialogService`、五对对话框 VM/Window、取色行为 `SpectrumCanvasBehavior`）驻
...
- **对外契约**：领域数据经注入提供者/模块出口获得——程序扫描候选经 SDK 契约
  ...
  （M1/M2/M5/Host）只显式引用 `StarPie.Sdk` 调 `IDialogService`。
- **扩展局部性**：新增对话框（原型 C）→ S6 内部 + 调用方一行；新增结果 record/对话框契约 →
  扩展 `StarPie.Sdk` 的 Dialogs 契约。
...
- **R4**：`MainView` 全文件归 H1 宿主壳，xaml.cs 不归 M5；页面 DataTemplate 不在 MainView——各模块模板字典随所属模块（见 [assemblies.md](assemblies.md) §2/§5.1）。
- **R6 三分**：图标资产 → S1（契约与资产表 `IconCatalog.cs`/`CustomIconItem.cs`/`VectorIconItem.cs`，双形拆为静态目录 + 实例服务，见 §3 S1）；几何（`CreateAdvancedSectorGeometry`/`GetCoreIconGeometry`）→ M2（`StarPie.Ui/Services/Wheel/WheelGeometry.cs`）；程序侧（`ResolveShortcutTarget`）→ M3（`StarPie.Host/Programs/ShortcutResolver.cs`）。
- **R8 Models 语义归属**：`WheelProfile`/`ActionItem` → M1 语义（物理 `StarPie.Sdk/Models/`，配置 POCO）；`WheelPalette*` → M2 语义（物理 `StarPie.Host/Wheel/`，WPF-free）；`CustomColorPreset` → M2 语义（物理 `StarPie.Sdk/Models/`，`AppConfig.CustomColorPresets` 配置 POCO 引用）。
- **R9**：`NavigationStore`、`NavigationExecutor`（含 `INavigationExecutor`）、`MainViewModel`、`NavigationItemViewModel`；与 R4/D3 同判据——运行时消费者全在 Host，模块程序集零引用；命名空间不变（共享命名空间树）。
...
| 原型/场景 | 示例 | 只动 |
|---|---|---|
| A 新增设置项 | 现有页加开关 | 所属模块 VM |
| B 新增设置页面 | 新导航页 | 新域/所属模块（贡献者 + 页面模板字典，见 [assemblies.md](assemblies.md) §5） |
| C 新增对话框 | 新模态 | S6 内部 |
| D 新增动作类型 | 新 Launch/Folder/Hotkey/System 值 | M1 内部（路由/执行/预设/槽位编辑/图标键映射） |
| E 新增轮盘样式 | 新 Renderer | M2 内部（渲染器/工厂/配色目录/外观选项） |
| F 新增后台服务/监听器 | 新 Hook/Service | 所属模块内部 |
| 附加：新语言 | — | S3 |
| 附加：新消息/通知类型 | — | S4 |
| 附加：新图标资产 | — | S1 |
| 附加：新主题方案 | — | M4 |
...
**放行共享面**（按原型对号；放行面定义见 §2.3）：

- **A**：S2 模型字段、S3 文案键。
- **B**：M5/M1 只动 Ui 集内各自部件（`ShellContributor`/`GesturesContributor` 的 RegisterNavigation/RegisterServices + 对应页面模板字典 + 页面 VM/View），不碰 Host；Host 外观聚合页经 `HostPageContributor` + 模板字典与 VM 登记；新增页面不碰其它模块，仅新增模块才 H1 登记；S3 文案。
- **C**：调用方模块一行（经 `IDialogService`）。
- **D**：新图标资产 → S1；S3 文案；config 兼容。
- **E**：S3 文案。
- **F**：所属贡献者一行；新增模块才 H1。
- **附加项**：新消息/通知类型、新图标资产走放行共享面；新主题方案 → S3 文案；新语言、新程序来源无放行面。
