...
> + 尚未归并的旧集（设计期投影壳 `StarPie.Core` 与两个模块 runtime
> `StarPie.Dialogs`/`StarPie.Shell`，划分见 §2）。
...
| `StarPie.Dialogs` | 类库 | S6 对话框实现：DialogService、五对对话框 VM/Window、SpectrumCanvasBehavior（契约 `IDialogService` + 结果 record 驻 `StarPie.Sdk`，ADR-0023；Dialogs → Sdk（对话契约与程序扫描/.lnk 契约）+ Sdk.Wpf（图标资产与主题契约面）+ `StarPie.Host`（S2/S3 与图标目录/程序扫描实现）单向，不引用任何其它模块 runtime；`StarPie.Core` 引用仅设计期资源锚 pack URI，非运行时依赖） |
...
StarPie (Ui/exe) ──→ StarPie.Sdk + StarPie.Sdk.Wpf + StarPie.Host + StarPie.Core（仅设计期资源锚）
     ...
     已撤销（M1 手势内核/动作路由归 Host、WPF 亲和件与 VM/View 归 Ui；M2 配色目录与解析归
     Host、VM/窗口/渲染器/工厂/转换器归 Ui；M4 引擎归 Host、字典/VM 归 Ui；S1/M3 的 WPF-free
     逻辑归 Host、图像构造归 Ui；契约归 SDK/Sdk.Wpf）。
```
...
调用（Shell/Host csproj 显式引用 SDK，M1/M2 已并入 Ui 集），实现由 DialogsModuleRegistrar 注册，
...
- e2e `AutomationId` 沿用 `NavPage0..3`，随槽位稳定。
  ...
  在 `StarPie.Shell`，其模板字典仍经跨程序集 pack URI 合并；M1 归并后
  `GesturesModuleRegistrar` 与 `GesturesPageTemplates.xaml` 在 Ui 集，模板字典本地合并；exe 内还承载
  Host 外观聚合页的 HostModuleRegistrar/HostPageTemplates（留 Host）。
...
- **注册自治**：每个业务程序集暴露注册器（公开静态类，含 `RegisterServices(IServiceCollection)` 与 `RegisterNavigation(NavigationCatalog)`）；组合根（`Composition`，驻 Ui 集）按固定顺序调用——`ShellModuleRegistrar`（M5：页面 VM 注册 + RegisterNavigation）、`ThemeModuleRegistrar`（M4，驻 Ui：主题服务与主题设置子 VM 注册；无导航页）、`WheelModuleRegistrar`（M2，驻 Ui：轮盘工厂 `IWheelFactory→WheelFactory` 与轮盘外观设置子 VM 注册；无导航页）、`GesturesModuleRegistrar`（M1，驻 Ui：手势管线、触发+手势两页 VM 与 `IProfilePreviewSource` 别名注册，RegisterNavigation 自报槽位 0/2）、`DialogsModuleRegistrar`（S6）；S1/M3 已归并撤销，其服务由组合根直登记（两模块无注册器、无导航页）。
- **M3（程序扫描，已归并）**：组合根直登记
