...
> (`docs/architecture/*.md`)为准,冲突时叶子优先。
...
| `StarPie.Host` | 类库(net10.0) | 宿主内核:配置 / 本地化 / 系统集成、图标与程序扫描、主题引擎、轮盘与动作、插件运行时 |

- **`StarPie`(项目 `StarPie.Ui`,WinExe)**:
  - **入口与外框**:`App` / `ResidentShell` / `SettingsConsole` / `Composition` / `DevInstance`;Host 窗口外框(`SettingsConsoleWindow` 全文件 + `WindowChromeViewModel`);导航运行时主体(`Services/Navigation/`:NavigationStore / NavigationExecutor(含 INavigationExecutor);`ViewModels/Navigation/`:NavigationViewModel / NavigationItemViewModel,命名空间 `StarPie.Ui.ViewModels.Navigation`)。
  ...
  - **模块件**:
    ...
    - M4:`Services/Themes/`(`IThemeService` 实现 `ThemeService`)、`Adapters/`(调色板适配器 `AppThemePaletteManager`)、`Themes/`(五套主题字典)、主题设置子 VM 与 `ThemeContributor`;
    ...
    - M1:`Services/Input/`(MouseInputHook 捕获与抑制 / ReplayWindow 回放窗口 / HookWatchdog 看门狗 / SystemCursor 探针;ADR-0052)、`Services/Actions/`(IActionExecutorService / ActionExecutorService)、`ViewModels/Pages|WheelInteraction/`(BehaviorSettingsViewModel / ProfileListViewModel / SlotViewModel)、`Views/Pages|Controls|Styles|DesignTime/`(触发 + 轮盘页、热键录制控件与样式字典)与 `WheelInteractionContributor` + `WheelInteractionPageTemplates.xaml`;
    - M5:`Services/SystemIntegration/`(TrayIconManager / TrayMenuEntry 与插件菜单合成 `TrayMenuComposer`)、`ViewModels/Pages/`(GeneralSettingsViewModel)、`Views/Pages/`(AdvancedSettingsPage)与 `SystemIntegrationContributor` + `SystemIntegrationPageTemplates.xaml`;
...
- **`StarPie.Sdk`(类库,net10.0)**:`Models/`(AppConfig / WheelProfile / ActionItem / CustomColorPreset / ColorMath / ScreenPoint);`Services/`(AppHostDelegates、Messages、Navigation 目录与槽位契约、Dialogs 契约与 6 结果 record、IWheelFactory、Themes 界面主题名目录 AppThemeNames、Icons 条目类型与 .lnk SPI、Programs 扫描契约与纯规则);`ViewModels/`(Pages 预览源接口、Wheel 轮盘只读接口)。命名空间根为工程名（`StarPie.Sdk.*`）。
...
- **`StarPie.Host`(类库,net10.0)**:宿主内核(零 WPF、零 XAML)——`Configuration`(S2 配置读写 / 防抖落盘接缝 / AppDataPaths)、`Localization`(S3 本地化实现 + `Strings*.resx` 四语言)、`SystemIntegration`(M5 开机自启注册表 + 内存整理,命名空间 `StarPie.Host.SystemIntegration`)、`Icons/`(`IconCatalog` 静态纯目录 + `CustomIconStore` 自定义图标目录,命名空间 `StarPie.Host.Icons`)、`Programs/`(内置来源 `ProgramScanner` + 程序来源能力契约 / 聚合 + `ShortcutResolver`,命名空间 `StarPie.Host.Programs`)、`Themes/`(`ThemeEngine`,命名空间 `StarPie.Host.Themes`)、`Ports/`(`IThemeApplier`,命名空间 `StarPie.Host.Ports`)、`Wheel/`(`WheelPalette` / `WheelPaletteCatalog` / `WheelPaletteParser`,命名空间 `StarPie.Host.Wheel`)、`WheelInteraction/`(`WheelInteractionEngine` / `WheelInteractionState` / `WheelInteractionReleaseResult` / `IWindowContext` / `WindowContext`,命名空间 `StarPie.Host.WheelInteraction`)、`Actions/`(`ActionRouting` + `ActionRoute` / `KeyStroke` / `SystemCommand`,命名空间 `StarPie.Host.Actions`)、`HostServices/`(插件可见宿主服务与每插件作用域)、`PluginRuntime/`(插件发现 / 清单 / 准入 / 状态 / 运行时 / 装载 / 卸载 / 生命周期 / 能力表 / 诊断)。件件可 headless 直接构造(Windows-only 项以 `[SupportedOSPlatform]` 标注)。
...
| 导航运行时 / 状态 | H1(宿主侧,与 R4 / D3 同判据——单一消费方在 Host) | StarPie(exe) | NavigationStore、NavigationExecutor(含 INavigationExecutor)、NavigationViewModel、NavigationItemViewModel(命名空间 `StarPie.Ui.*`) |
| 系统集成与常驻服务 | M5 | `StarPie.Ui`(+ `StarPie.Host/SystemIntegration`) | 托盘、自启、内存、Advanced 设置面 |
| Host 窗口外框 | H1(宿主侧) | StarPie(exe) | SettingsConsoleWindow 全文件、WindowChromeViewModel、SettingsConsole / ResidentShell / App / Composition(导航 VM 与主框架同窗,物理同居 Host) |

`SettingsConsoleWindow.xaml.cs` 与 `WindowChromeViewModel` **归 Host 窗口外框**;M5 只拥有系统集成服务与设置面。
导航运行时(含导航区 VM `NavigationViewModel`)与窗口外框同判据归 Host(R9)。
...
- Host 提供导航执行入口(`NavigationExecutor` / `INavigationExecutor`):归 H1,为宿主内部件
  (非跨程序集"已批准解析缝",见本文 §8;第二消费方出现时按模块契约归属判据裁决落点——
...
- e2e `AutomationId` 沿用 `NavPage0..4`,随槽位稳定;插件页 AutomationId 由宿主按
  ...
  as-built:`WheelInteractionContributor` / `SystemIntegrationContributor` 与其页面模板字典均驻
...
| 1 | `NavPage1` | `PageAppearance` | `AppearanceSettingsViewModel`(聚合页) | `AppearanceSettingsPage` | Host |
| 2 | `NavPage2` | `PageWheelActions` | `ProfileListViewModel` | `WheelInteractionSettingsPage` | Ui(M1) |
...
运行时主体(含 NavigationViewModel)在 Host、目录契约驻 `StarPie.Sdk`,程序集归属见 §2 / §4。

## 6. DI 与注册契约(as-built)

- **统一注册管线(内置贡献者与未来插件贡献者共用入口)**:`ICompositionContributor`(`Id` / `Order` / `RegisterServices(IServiceCollection)` / 可选 `RegisterNavigation(NavigationCatalog)`,末者以接口默认实现表达"无导航页")驻 `StarPie.Ui/Modules/`;`BuiltInContributors.CreateAll` 给出**有序清单**——`HostCoreContributor`(宿主编排与内核接入:宿主回调委托包、程序扫描 / .lnk、图标资产、配置 / 本地化 / 防抖、消息、导航运行时、窗口外框 VM、插件运行时与插件管理页;槽位 4)、`HostPageContributor`(槽位 1 聚合页)、`ThemeContributor`(M4:主题服务与主题设置子 VM;无导航页)、`WheelContributor`(M2:轮盘工厂 `IWheelFactory → WheelFactory` 与外观设置子 VM;无导航页)、`WheelInteractionContributor`(M1:轮盘交互管线、两页 VM 与 `IProfilePreviewSource` 别名;槽位 0 / 2)、`SystemIntegrationContributor`(M5:高级页 VM;槽位 3)、`DialogsContributor`(S6:`IDialogService → DialogService`;无导航页);S1 / M3 无导航页,其服务由 `HostCoreContributor` 登记。
...
- **根解析集中**:Host Composition 唯一 `BuildServiceProvider` / `CreateResidentShell`;贡献者不解析、不持容器。
...
- `AppHostDelegates` 是 SDK 公开契约(`StarPie.Sdk/Services/AppHostDelegates.cs`);
  Host 组合根持有实例并由 `HostCoreContributor` 登记单例,ResidentShell 构造后回填实现(契约名不随类改名)。贡献者只依赖
...
- 跨集必需的内部件(如 `TrayIconManager`)单独裁决为 public 或经接口注入。
  ...
  调用方(Host 外观页)以 `bool` 传入(M2 不反向引用 Host `SettingsConsoleWindow`)。
...
| 契约缝·主题 | `IThemeService` 驻 `StarPie.Sdk.Wpf/Services/Themes/` ← 实现 `ThemeService` 驻 `StarPie.Ui/Services/Themes/`(状态 / 解析在内核 `ThemeEngine`,换肤经端口 `IThemeApplier` 回抛 Ui);消费方 Host / Dialogs 经契约边(M2 轮盘侧不经本契约,改经无状态 `Func<bool>` 探针) | xUnit;**不构成插件可达面**——插件拿不到本服务实例(无注入边),插件侧深浅色同样走探针 |
...
| 契约缝·预览 Profile | `IProfilePreviewSource` 驻 `StarPie.Sdk`(生产方语义 + 破 Wheel↔WheelInteraction 环),别名 = M1 `ProfileListViewModel`,消费 M2 经契约边 | D5 + xUnit |
...
| 注册缝 | 统一注册管线:`ICompositionContributor`(`Id` / `Order` / `RegisterServices` + 可选 `RegisterNavigation`)+ `BuiltInContributors` 有序清单(`HostCoreContributor` / `HostPageContributor` / `ThemeContributor` / `WheelContributor` / `WheelInteractionContributor` / `SystemIntegrationContributor` / `DialogsContributor` 七个内置贡献者)下放 DI / 导航登记;注册的契约类型驻 `StarPie.Sdk` / `StarPie.Sdk.Wpf`;组合根唯一解析、插件贡献者接同一接口 | 原 xUnit:`BuiltInContributors` 已下线(见 ADR-0050) |
...
| 缝 | 位置 | 压力 | 裁决 / 触发条件 |
|---|---|---|---|
| Host 装配面 | Composition / CreateResidentShell 直取 Host 侧可见具体类型(MouseInputHook / 主题服务 / 两子 VM 等);ResidentShell 把 Ui 侧调色板适配器接到主题服务(内核端口 `IThemeApplier`)/ 编排托盘菜单 / MouseInputHook 暂停态;Host 聚合页拼装 M2 / M4 子 VM | Host 对"哪些装配件可见"有编译期认知;模块不能脱离 Host 决定宿主装配 | 留 Host;不引入子容器 / Prism |
