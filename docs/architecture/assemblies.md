...
- **注册顺序 ≠ 解析时机**：组合根（`Composition`，驻 Ui 集）按四阶段显式分离——(1) 早期回填（`AppDataPaths.IsDevInstance`，注册前）→ (2) 注册期（有序清单先写导航目录并 `Validate()` 收口五槽，再写容器描述符）→ (3) 唯一 `BuildServiceProvider` → (4) `CreateAppHost` 在配置加载后目录驱动 eager 解析全部页面 VM 与宿主直持 VM。贡献者只登记不解析，解析点只在组合根。
