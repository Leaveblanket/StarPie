...
验证义务分两层门：**提交级**与**合入门**。xUnit 全量便宜（MTP 报告执行约 3–5 秒），任何代码提交都全量跑，不按模块拆分；e2e 用免跑判定，不拆用例子集、不移除每例冷启动。拿不准时跑全量。

**不涉及代码变动的提交免除全部验证**：只改非代码文件（`docs/**`、`*.md`、`.out-of-scope/**` 等）、既不触碰代码也不触碰工程与资源文件的提交，build、xUnit 与 e2e 一概不跑——文档不参与编译与运行，三者都不会因它改变，跑只是等待；合入门同理。判定看的是**改了什么**而不是提交类型（`docs` 提交改了 `Strings.resx` 仍按代码处理），拿不准时按“涉及代码”处理。

> 为什么这样分层：xUnit 全量执行 3–5 秒，按模块拆子集没有可省空间且归属映射需持续维护；e2e 每用例冷启动保证独立沙箱与默认配置确定性（多数用例写穿 config 不还原），移除重启会引入顺序依赖与单例崩溃级联失败，省时收益有限。全量兜底放在合入门，CI 只强制 xUnit（GitHub runner 上 WPF UI 自动化可靠性不足）。

1. **Build must pass**（每个涉及代码的提交前）:
...
3. **pywinauto e2e（提交级免跑判定）**: feature / bug-fix 提交按下表判定，命中“必跑”时才先 build 再全量跑：
   ...
   运行器默认静默形态（被测应用以 `--background` 启动：屏幕左上角、不可激活、点击穿透、不进任务栏，
   Save 提示框不呈现），全程不抢前台、不移动物理光标；日志与 junitxml 落 `artifacts/e2e/`，
   `-Status` 查最近一次结果（退出码 0=通过、非 0=失败、3=仍在跑、4=无记录），
   `-OnScreen` 为调试用可见形态（可激活、对话框真实呈现）。等价裸命令见 `tests/conftest.py`。
...
4. **合入门（merge 到 main 前）**: 主干同步后至少一次全量 xUnit + 一次全量 pywinauto e2e（agent 本地执行）；CI 只强制 xUnit。纯文档分支免除（无代码变动可验）。
