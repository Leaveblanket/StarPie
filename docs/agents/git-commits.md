...
验证义务分两层门：**提交级**与**合入门**。xUnit 全量便宜（当前 598 例，纯执行约 1.0s），任何代码提交都全量跑，不按模块拆分；e2e 用免跑判定，不拆用例子集、不移除每例冷启动。拿不准时跑全量。

> 为什么这样分层：xUnit 全量纯执行约 1 秒，按模块拆子集没有可省空间且归属映射需持续维护；e2e 每用例冷启动保证独立沙箱与默认配置确定性（多数用例写穿 config 不还原），移除重启会引入顺序依赖与单例崩溃级联失败，省时收益有限。全量兜底放在合入门，CI 只强制 xUnit（GitHub runner 上 WPF UI 自动化可靠性不足）。
